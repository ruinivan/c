...
void carregar_fase(int fase, int parede[10][35]) {
    // Fase 1
    if (fase == 0) {
        for (int i = 8; i < 16; i++) {
            parede[12][i] = 1;
...
void inicializar_moedas(moedas *moeda[], int quant_moeda, int parede[10][35]) {
    // Garante que temos as 50 moedas
    for (int i = 0; i < quant_moeda; i++) {
        moeda[i] = (moedas *)malloc(sizeof(moedas));
        do {
            moeda[i]->local.x = rand() % 9 + 1; // Gera valores entre 1 e 8 para x
            moeda[i]->local.y = rand() % 33 + 1; // Gera valores entre 1 e 33 para y
        } while (parede[moeda[i]->local.x][moeda[i]->local.y] == 1); // Evita colocar moedas em paredes
    }
}

void inicializar_frutas(frutas *fruta[], int quant_frutas, int parede[10][35]) {
...
void desenhar_mapa(int parede[10][35], fantasmas fantasma[], moedas *moeda[], frutas *fruta[], int quant_moeda, int quant_frutas) {
...
void mover_pacman(int parede[10][35]) {
    if (_kbhit()) {
        char tecla = _getch();
        switch (tecla) {
            case 'w': // Cima
                if (pacman.local.x > 0 && parede[pacman.local.x - 1][pacman.local.y] == 0)
                    while(tecla == 'w') pacman.local.x--;
                break;
            case 'a': // Esquerda
...
void mover_fantasmas(fantasmas *fantasmas, int num_fantasmas, int parede[10][35]) {
...
int main() {
    ...
    while (fase < 5) {
        ...
        Sleep(200); // Tempo entre as atualizações (suave para o console)
    }
